...
ans = json.load(open("./CoNLL_Sample_ans.json", "r"))
...
print("= " * 15 + "WHOLE" + " =" * 15, "\n")
print("### Close:")
ans = json.load(open("./CoNLL_All_ans.json", "r"))

rigth_ls = list()
error_ls = list()

for line in ans:
    if line["close_pred"] == line["truth"]:
...
for item in ans:
    ...
    try:
        ...
        continue
    close_preds.append(p)
    close_target.append(label2id[item["truth"]])
close_preds, close_target = torch.tensor(close_preds), torch.tensor(close_target)
metric = MulticlassCalibrationError(num_classes=len(id2label), n_bins=50, norm='l1')
close_calib = metric(close_preds, close_target)
