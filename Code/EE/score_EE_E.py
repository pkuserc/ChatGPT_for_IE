...
def evaluate(preds_event, preds_arg, gold_event, gold_arg):
    ...
    for example_id in preds_event:
        for pred_tri in preds_event[example_id]:
            word_idx, event_type, trigger_word, tri_confidence = pred_tri
            match_idn = {item for item in gold_event[example_id] if item[0] == word_idx}
            match_word = [item for item in gold_event[example_id] if item[2] == trigger_word and item[1]==event_type]
            if match_word:
                match_word_num += 1
    ...
    for example_id in preds_arg:
        ...
        # preds_arg[example_id] = [tuple(i) for i in preds_arg[example_id]]
    ...
    for example_id in preds_arg:
        for pred_arg in preds_arg[example_id]:
            start, end, event_type, role, text, arg_confidence = pred_arg
...
def read_gold_example(path):
    ...
    for line in lines:
        ...
        for event in events:
            ...
            gold_event[line['id']].append([word_idx, event_type, trigger_word])
...
def main(result_dir, gold_path):
    ...
    for file in os.listdir(result_dir):
        ...
        for event in res:
            ...
            preds_event[example_id].append([word_idx, event_type, trigger_word, tri_confidence])
            ...
            for arg in event['participants']:
                ...
                preds_arg[example_id].append([start, end, event_type, role, text, arg_confidence])
...
if __name__ == "__main__":
    result_dir = './Output/EE/Full_Testset/EE_E_Closed'
    gold_path = './data/ACE05-E/EE_E_gold.json'
